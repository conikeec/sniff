...
//! the claude-tree application for representing Claude Code sessions,
...
impl ClaudeMessage {
    /// Returns the base message information.
    pub fn base(&self) -> &MessageBase {
        match self {
            ClaudeMessage::User(msg) => &msg.base,
            ClaudeMessage::Assistant(msg) => &msg.base,
        }
    }
    ...
    /// Returns the current working directory for this message.
        ...
        match self {
            ...
            ClaudeMessage::Assistant(msg) => msg
                ...
                .filter_map(|item| match item {
                    ...
                    AssistantContentItem::Text { .. } => None,
                })
                .collect(),
        }
    }
