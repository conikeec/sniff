...
impl BullshitDisplayFormatter {
    ...
    fn format_code_context(&self, context: &ContextLines, _detection: &BullshitDetection) -> String {
        let mut output = String::new();
        let term_width = self.get_current_terminal_width();
    ...
    pub fn format_file_header(&self, file_path: &str, detection_count: usize) -> String {
        ...
        if detection_count == 0 {
            if self.use_colors {
                ...
                format!("✅ {} - No issues found", file_name)
            }
        } else {
            if self.use_colors {
                format!(
    ...
    fn format_minimal_context(&self, context: &ContextLines, line_num_width: usize) -> String {
        ...
        if self.use_colors {
            output.push_str(&format!(
                "   {} │ {}\n",
                format!("{:width$}", target_line_num, width = line_num_width).red().bold(),
    ...
    pub fn format_file_summary_adaptive(&self, file_path: &str, detections: &[BullshitDetection]) -> String {
        let term_width = self.get_current_terminal_width();
        ...
        if term_width < 80 {
            ...
            let most_severe = detections.iter()
    ...
    pub fn format_summary_tree(&self, file_summaries: &[(String, Vec<BullshitDetection>)]) -> String {
        let mut output = String::new();
        ...
        for (i, (file_path, detections)) in file_summaries.iter().enumerate() {
            ...
            if detections.is_empty() {
                ...
                let critical_count = detections.iter().filter(|d| matches!(d.severity, Severity::Critical)).count();
                let high_count = detections.iter().filter(|d| matches!(d.severity, Severity::High)).count();
